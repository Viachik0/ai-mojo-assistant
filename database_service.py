...
from app.core.database import AsyncSession as AsyncSessionMaker
...
class DatabaseService:
    """Service for querying and analyzing educational data from the database"""
    ...
    async def get_students_by_class(self, class_name: str) -> List[Student]:
        async with AsyncSessionMaker() as session:
            result = await session.execute(select(Student).where(Student.class_name == class_name))
            return result.scalars().all()
    ...
    async def get_grades_for_student(self, student_id: int, days: int = 7) -> List[Grade]:
        async with AsyncSessionMaker() as session:
            ...
            result = await session.execute(select(Grade).where(Grade.student_id == student_id, Grade.date >= since))
            return result.scalars().all()
